...
function LibraryView({ items, onSave, onDelete, onLog, saving }:{onLog:(itemId:string)=>void;items:LibraryItem[];onSave:(item:LibraryItem)=>Promise<boolean>;onDelete:(itemId:string)=>void;saving:boolean}) {
  ...
  async function saveReceipt(){
    ...
    try{for(const food of receiptItems.filter(item=>item.selected&&item.name.trim())){
    ...
    }setReceiptMessage(`${savedCount} food${savedCount===1?'':'s'} added as library clues.`);}finally{setReceiptBusy(false);}
  }
  ...
  return <>
    ...
    <section className="research-card"><h2>Add foods from a receipt</h2><p>Upload a receipt to remember groceries and snacks. Only clearly identified food and drink items are suggested. Nutrition is looked up separately.</p><label>Receipt photo<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={receiptBusy||saving} onChange={event=>{const file=event.target.files?.[0];if(file)uploadReceipt(file);event.target.value='';}}/></label>{receiptMessage&&<p role="status">{receiptMessage}</p>}{receiptItems.map((item,index)=><div className="research-search receipt-food-row" key={index}><input type="checkbox" aria-label={`Include ${item.name}`} checked={item.selected} disabled={receiptBusy} onChange={event=>setReceiptItems(all=>all.map((food,i)=>i===index?{...food,selected:event.target.checked}:food))}/><input aria-label={`Receipt food ${index+1}`} value={item.name} maxLength={200} disabled={receiptBusy} onChange={event=>setReceiptItems(all=>all.map((food,i)=>i===index?{...food,name:event.target.value}:food))}/></div>)}{receiptItems.length>0&&<button className="primary" disabled={receiptBusy||saving||!receiptItems.some(item=>item.selected&&item.name.trim())} onClick={saveReceipt}>{receiptBusy?'Saving…':'Add selected foods to Library'}</button>}</section>

...
function LibraryView({ items, onSave, onDelete, onLog, saving }:{onLog:(itemId:string)=>void;items:LibraryItem[];onSave:(item:LibraryItem)=>void;onDelete:(itemId:string)=>void;saving:boolean}) {
  ...
  return <>
    ...
    {items.length?<><div className="section-title"><div><h2>Saved foods</h2><span>{items.length} in your library</span></div></div><div className="library-grid">{items.map(item=><article key={item.id} className="library-card"><div className={`library-icon ${item.kind}`}>{item.kind==='recipe'?'R':item.kind==='meal'?'M':'F'}</div><span className="soft-pill">{item.kind}</span><h3>{item.name}</h3>{item.components?.length? <details className="saved-components"><summary>{item.components.length} saved ingredients</summary>{item.components.map((part,index)=><div key={`${part.id}-${index}`}><strong>{part.name}</strong><span>{part.quantity} {part.unit} · {round(part.calories)} kcal</span></div>)}<p>Log this meal to edit individual ingredients without changing this saved group.</p></details>:null}<p>{item.quantity} {item.unit} · {round(item.calories)} kcal</p>{item.servingsPerCookedCup&&<p className="conversion-note">1 cooked cup = {item.servingsPerCookedCup} serving{item.servingsPerCookedCup===1?'':'s'}</p>}{item.alias&&<blockquote>Matches “{item.alias}”</blockquote>}<div className="library-macros"><span>P {round(item.protein)}g</span><span>C {round(item.carbs)}g</span><span>F {round(item.fat)}g</span></div><button className="primary" disabled={saving} onClick={()=>onLog(item.id)}>Log to today</button><div className="library-links">{item.sourceUrl&&<a className="library-source" href={item.sourceUrl} target="_blank" rel="noreferrer">{item.sourceLabel} ↗</a>}<button disabled={saving} onClick={()=>onDelete(item.id)}>Remove</button></div></article>)}</div></>:!adding&&<div className="all-clear"><span>◇</span><h2>Your library is ready</h2><p>Research a branded food above, or add a custom food manually.</p></div>}
  </>;
}
